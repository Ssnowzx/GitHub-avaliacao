...
import { analyzeGitHubProfile, generateProfileReadme } from './services/geminiService';
...
const App: React.FC = () => {
    ...
    const handleAnalysis = useCallback(async (url: string) => {
        ...
        try {
            ...
            if (parsedUrl.hostname !== 'github.com' || parsedUrl.pathname.split('/').filter(Boolean).length !== 1) {
        ...
        try {
            ...
            let analysisContent = result;

            // Extract star rating and justification
            const ratingMatch = result.match(/RATING:\s*(\d)\/5\s*(?:\(([^)]+)\))?/);
            if (ratingMatch && ratingMatch[1]) {
                setStarRating(parseInt(ratingMatch[1], 10));
                if (ratingMatch[2]) {
                    setRatingJustification(ratingMatch[2]);
                }
                analysisContent = analysisContent.replace(/RATING:\s*\d\/5\s*(?:\([^)]+\))?/, '').trim();
            }

            // Extract technologies
            const techMatch = result.match(/TECHS:\s*\[([^\]]+)\]/i);
             if (techMatch && techMatch[1]) {
                setTechnologies(techMatch[1].split(',').map(tech => tech.trim().toLowerCase()));
                analysisContent = analysisContent.replace(/TECHS:\s*\[[^\]]+\]\s*/i, '').trim();
            }
    ...
    const handleGenerateReadme = useCallback(async () => {
        ...
        try {
            const readme = await generateProfileReadme(analysis, language);
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    {analysis && !isLoading && (
                        <div className="space-y-8 animate-fade-in">
                            {starRating > 0 && <StarRating rating={starRating} technologies={technologies} title={t.overallRating} justification={ratingJustification} />}
                            <AnalysisDisplay content={analysis} />

                            <div className="text-center pt-4">
                                <h2 className="text-xl md:text-2xl font-semibold mb-4">{t.readmeTitle}</h2>
                            ...
                            {readmeError && <ErrorMessage message={readmeError} title={t.errorTitle} />}
                            {readmeContent && !isReadmeLoading && <ReadmeDisplay content={readmeContent} copyText={t.copyButton} copiedText={t.copiedButton}/>}
                        </div>
                    )}
                </div>
            </main>
